...
type SettingsView = 'list' | 'unlock' | 'deepseek'
...
const unlockCodeStorageKey = 'valkophoneUnlockCode:v1'
const unlockCodeHashStorageKey = 'valkophoneUnlockCodeHash:v1'
const unlockCodeLabelStorageKey = 'valkophoneUnlockCodeLabel:v1'
...
function maskRedemptionCode(code: string) {
  if (!code) {
    return ''
  }
...
function App() {
  ...
  const redeemCode = async (code: string) => {
    ...
    window.localStorage.setItem(unlockCodeStorageKey, result.normalizedCode)
    window.localStorage.setItem(unlockCodeHashStorageKey, result.codeHash)
    window.localStorage.setItem(unlockCodeLabelStorageKey, maskRedemptionCode(result.normalizedCode))
...
function SettingsApp({
  ...
  const unlockCodeLabel = readLocalStorage(unlockCodeLabelStorageKey, isUnlocked && draftCode ? maskRedemptionCode(draftCode) : '')
  ...
  return (
    <section className="settings-shell" aria-label="设置">
      <header className="app-topbar">
        <button className="icon-button" type="button" aria-label={settingsView === 'list' ? '返回桌面' : '返回设置'} onClick={goBack}>
          <ArrowLeft size={21} strokeWidth={2.4} />
        </button>
        <h2>{title}</h2>
        <span />
